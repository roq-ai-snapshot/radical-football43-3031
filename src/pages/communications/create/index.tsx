...
function CommunicationCreatePage() {
  ...
  const formik = useFormik<CommunicationInterface>({
    initialValues: {
      message: '',
      date_sent: new Date(new Date().toDateString()),
    ...
    <AppLayout>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="date_sent" mb="4">
            <FormLabel>date_sent</FormLabel>
            <DatePicker
              dateFormat={'dd/MM/yyyy'}
              selected={formik.values?.date_sent}
              onChange={(value: Date) => formik.setFieldValue('date_sent', value)}
            />
          </FormControl>
