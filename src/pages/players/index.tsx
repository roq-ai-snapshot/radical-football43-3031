...
function PlayerListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlayerInterface[]>(
    ...
    () =>
      getPlayers({
        relations: ['user', 'academy', 'development_goal.count', 'performance_data.count', 'training_exercise.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr key={record.id}>
                    ...
                    {hasAccess('training_exercise', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.training_exercise}</Td>
                    )}
