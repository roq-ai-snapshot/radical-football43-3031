...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createPlayer() {
    ...
    if (body?.training_exercise?.length > 0) {
      const create_training_exercise = body.training_exercise;
      body.training_exercise = {
        create: create_training_exercise,
      };
    } else {
      delete body.training_exercise;
    }
